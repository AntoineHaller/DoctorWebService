...
@router.get("/{id}", response_model=Patient, description="Get one patients with Id")
def get_patient(id: str):
    return conn.execute(patients.select().where(patients.c.id == id)).first()

@router.get("", response_model=List[Patient], description="Get all patients")
def get_patients():
    return conn.execute(patients.select()).fetchall()


@router.post("", response_model=Patient, description="Create patient")
