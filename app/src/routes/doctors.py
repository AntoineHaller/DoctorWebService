...
@router.get("/{id}", response_model=Doctor, description="Get one single doctor with the id")
def get_doctors(id: str):
    return conn.execute(doctors.select().where(doctors.c.id == id)).first()

@router.get("",response_model=List[Doctor],description="Get all doctors")
def get_doctors():
    return conn.execute(doctors.select()).fetchall()

@router.get("/{speciality}/{city}", response_model=List[Doctor], description="Get one single doctor with his speciality and city")
...
@router.post("", response_model=Doctor, description="Create doctor")
...
@router.put("/{id}", response_model=Doctor, description="Update doctor with Id")
